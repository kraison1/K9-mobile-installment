...
import { fetchInfoProductSavingPayMentImages } from "src/store/productSavingPayMentImage";
...
const ModalBank = ({ open, setModal, RowData, submitRow }) => {
  ...
  React.useEffect(() => {
    if (open && modalRef.current) {
      ...
      if (isEmpty(store.selectPaySaving)) {
        dispatch(fetchSelectBank("5"));
      }
    }
  }, [open]);
  ...
  React.useEffect(() => {
    if (!isEmpty(store.selectPaySaving)) {
      setBanks(
        store.selectPaySaving.map((item) => ({
          value: item.id,
  ...
  }, [store.selectPaySaving]);

  const fetchInfo = (id) => {
    setIsLoadingOpen(true);
    dispatch(fetchInfoProductSavingPayMentImages(id))
      .unwrap()
      ...
      >
        <div className="bg-white rounded-lg shadow overflow-y-auto max-h-[95vh]">
          ...
          <div className="p-2 grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="  justify-items-end col-span-2 lg:col-span-2">
              <label
                htmlFor="datePay"
