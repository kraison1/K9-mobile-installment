...
import { fetchInfoProcessSaving } from "src/store/processSaving";
...
const ModalProcessSaving = ({ open, setModal, RowData, submitRow }) => {
  const { isLoadingOpen, setIsLoadingOpen } = useAuth();
  const dispatch = useDispatch();
  const store = useSelector((state) => state.processSaving);
  ...
  React.useEffect(() => {
    const processSaving = store.data;

    if (processSaving) {
      setCurrentStatus(processSaving.status);
      reset({
        ...processSaving,
        status: processSaving.status == "1" ? "2" : processSaving.status,
      });
    }
  }, [store]);
  ...
  const fetchInfo = (id) => {
    ...
    dispatch(fetchInfoProcessSaving(id))
      .unwrap()
  ...
  const onSubmit = (data) => {
    conFirm(
      `ยืนยันการปิดจอง ${RowData.productSaving.code}`,
      "ตกลง",
      "ปิด",
      true
    ).then((e) => {
          ...
          <div className="flex items-center justify-between p-3 border-b rounded-t">
            <h3 className="text-xl font-semibold text-gray-900">
              {`${
                isNaN(RowData.id)
                  ? "เพิ่มรายการใหม่"
                  : `แก้ไขรายการ: ${RowData.productSaving.code}`
              }`}
            </h3>
            <button
              type="button"
          ...
          <fieldset disabled={CurrentStatus == 1 ? false : true}>
            <div className="p-2 grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="items-center col-span-3">
                <label
                  htmlFor="uploadFilProcessSaving"
                  className="block text-sm font-medium text-gray-700 mr-2"
                >
              ...
              {!isEmpty(watch("processSavingImages")) ? (
                <div className="items-center col-span-3">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2 lg:col-span-2">
                      <DragAndDropImages
                        images={watch("processSavingImages")}
                        submitSwitch={() => {}}
                        onDelete={() => {}}
                        showDelete={false}
                      />
...
export default ModalProcessSaving;
