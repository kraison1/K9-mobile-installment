...
import { fetchInfoProductBook } from "src/store/productBook";
...
const ModalBook = ({ open, setModal, RowData, submitRow }) => {
  const { user, isLoadingOpen, setIsLoadingOpen, permissions } = useAuth();
  const dispatch = useDispatch();
  const store = useSelector((state) => state.productBook);
      ...
      if (user.type != "admin-external") {
        dispatch(
          fetchSelectProduct({
            ...
            active: RowData.id == undefined ? "1" : "6",
          })
        );
      }
  ...
  const fetchInfo = (id) => {
    ...
    dispatch(fetchInfoProductBook(id))
      .unwrap()
        ...
        <div className="bg-white rounded-lg shadow overflow-y-auto max-h-[95vh]">
          ...
          <fieldset disabled={RowData.id ? true : false}>
            <div className="p-2 grid grid-cols-1 lg:grid-cols-4 gap-4">
              <div className="justify-items-end col-span-4 lg:col-span-4">
                <label
                  htmlFor="caseDate"
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <DatePicker
                      ...
                      minDate={dayjs().subtract(10, "day").toDate()} // ห้ามเลือกวันที่ก่อน 7 วันจากปัจจุบัน
                      maxDate={dayjs().add(10, "day").toDate()} // ห้ามเลือกวันที่7 วันจากปัจจุบัน
                    />
                  )}
                />
              </div>
              ...
              {!isEmpty(watch("productBookImages")) ? (
                <div className="items-center col-span-4">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2 lg:col-span-2">
                      <DragAndDropImages
                        images={watch("productBookImages")}
                        submitSwitch={() => {}}
                        onDelete={() => {}}
                        showDelete={false}
                      />
