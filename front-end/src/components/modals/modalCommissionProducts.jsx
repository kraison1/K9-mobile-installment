...
import { fetchInfoCommissionProducts } from "src/store/commissionProduct";
...
const ModalCommissionProducts = ({ open, setModal, RowData, submitRow }) => {
  ...
  const storeCommissionProduct = useSelector((state) => state.productModel);
  const storeProductStorage = useSelector((state) => state.productStorage);

  React.useEffect(() => {
    if (!isEmpty(storeCommissionProduct.select)) {
      setProductModels(
        storeCommissionProduct.select.map((item) => ({
          value: item.id,
          label: item.name,
          type: item.type,
        }))
  ...
  const fetchInfo = (id) => {
    ...
    dispatch(fetchInfoCommissionProducts(id))
      .unwrap()
        ...
        <div className="bg-white rounded-lg shadow overflow-y-auto max-h-[95vh]">
          ...
          <div className="p-2 grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="items-center sm:col-span-2 lg:col-span-2">
              ...
              {errors.priceCommission && (
                <span className="text-red-500 text-xs mt-1">
                  กรุณาใส่ข้อมูล ค่าคอมพนักงานปรับใหม่
                </span>
              )}
            </div>
                ...
                render={({ field }) => (
                  <div>
                    <input
                      ...
                      className="mr-2"
                    />
                    <label htmlFor="hand-2" className="text-red-500">
                      มือหนึ่ง
                    </label>
                    <input
                      {...field}
...
ModalCommissionProducts.propTypes = {
  open: PropTypes.bool,
...
export default ModalCommissionProducts;
