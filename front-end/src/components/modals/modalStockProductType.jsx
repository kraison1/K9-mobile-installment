...
import { fetchSelectProductUnits } from "src/store/productUnit";
import { fetchInfoProductTypes } from "src/store/productType";
import Select from "react-select";

const ModalStockProductType = ({ open, setModal, RowData, submitRow }) => {
  ...
  const storeProductUnit = useSelector((state) => state.productUnit);

  React.useEffect(() => {
    if (!isEmpty(storeProductUnit.select)) {
      setProductUnits(storeProductUnit.select);
    }
  }, [storeProductUnit.select]);

  React.useEffect(() => {
    if (open && modalRef.current) {
      if (isEmpty(storeProductUnit.select)) {
        dispatch(fetchSelectProductUnits());
      }
  ...
  const fetchInfo = (id) => {
    ...
    dispatch(fetchInfoProductTypes(id))
      .unwrap()
          ...
          <div className="p-2 grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="items-center col-span-1 ">
              ...
              <Controller
                ...
                rules={{ required: true }}
                render={({ field }) => {
                  const selectedValue = ProductUnits.find(
                  ...
                  return (
                    <Select
                      ...
                      placeholder="กรุณาเลือกหน่วยสินค้า"
                      isClearable
                      isSearchable
                ...
                render={({ field }) => (
                  <div>
                    <input
                      ...
                      type="radio"
                      value="มือถือ"
                      checked={field.value === "มือถือ"}
                      onChange={() => field.onChange("มือถือ")}
                      className="mr-2"
                    />
                    ...
                    <input
                      ...
                      type="radio"
                      value="อุปกรณ์เสริม"
                      checked={field.value === "อุปกรณ์เสริม"}
                      onChange={() => field.onChange("อุปกรณ์เสริม")}
                      className="mr-2 ml-4"
                    />
                    ...
                    <input
                      ...
                      type="radio"
                      value="อะไหล่ซ่อม"
                      checked={field.value === "อะไหล่ซ่อม"}
                      onChange={() => field.onChange("อะไหล่ซ่อม")}
                      className="mr-2 ml-4"
                    />
...
ModalStockProductType.propTypes = {
  open: PropTypes.bool,
...
export default ModalStockProductType;
