...
const TableBranchTransferPrice = ({
  tableHeaders,
  tableLists,
  onClick,
  page,
        ...
        <tbody>
          {tableLists.map((TableList, k) => (
            <tr className="bg-white" key={k}>
              ...
              <td className="border p-2 font-light">
                <p> {`โอนเข้าบัญชี: ${TableList.infoBank}`}</p>
                <p> {`ใช้บัญชี: ${TableList.fromInfoBank || "-"}`}</p>
              </td>

              <td className="border p-2 font-light">
                <p> {`โอนให้: ${TableList.branch?.name}`}</p>
                <p> {`จาก: ${TableList.fromBranch.name || "-"}`}</p>
              </td>
              <td className="border p-2 font-light text-left">
                {permissions.includes("can-deleted") ? (
                  ...
                  >
                    ...
                    <p> {formatDateTH(TableList.create_date)}</p>
                  </span>
                ) : (
...
TableBranchTransferPrice.propTypes = {
...
export default TableBranchTransferPrice;
