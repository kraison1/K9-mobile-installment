        ...
        <tbody>
          {tableLists.map((TableList, k) => (
            <tr className="bg-white" key={k}>
              ...
              >
                {`${TableList.product?.code} / ${TableList.product?.imei || '-'}`}
              </td>

              <td className="border p-2 font-light">{TableList.action}</td>
              <td className="border p-2 font-light">{TableList.user?.name}</td>
              <td className="border p-2 font-light">{formatDateTH(TableList.create_date)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
TableStockProductLog.propTypes = {
...
export default TableStockProductLog;
