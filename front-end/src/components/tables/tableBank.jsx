...
const TableBank = ({ tableHeaders, tableLists, onClick, page, pageSize }) => {
  return (
        ...
        <tbody>
          {tableLists.map((TableList, k) => (
            <tr className="bg-white" key={k}>
              ...
              >
                <p>{`ธนาคาร: ${TableList.bankName}`}</p>
                <p>{`เลขบัญชี: ${TableList.bankNo}`}</p>
                <p>{`ชื่อบัญชี: ${TableList.bankOwner}`}</p>
              </td>
              <td className="border p-2 font-light text-right">{`${formatNumberDigit(
                TableList.priceAll
              )}`}</td>
              <td className="border p-2 font-light text-right">{`${formatNumberDigit(
                TableList.priceCurrent
              )}/${formatNumberDigit(TableList.priceLimit)}`}</td>
            </tr>
          ))}
        </tbody>
      </table>
