        ...
        <tbody>
          {tableLists.map((TableList, k) => (
            <tr className="bg-white" key={k}>
              ...
              <td className="border p-2 font-light">{TableList.catalog}</td>
              <td className="border p-2 font-light">{`${
                isEmpty(TableList.productUnit) ? "" : TableList.productUnit.name
              }`}</td>
            </tr>
          ))}
        </tbody>
...
TableStockProductType.propTypes = {
...
export default TableStockProductType;
