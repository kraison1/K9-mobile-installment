...
import { contractProductSale, slipProductSale } from "src/store/productSale";
...
const TableProductSale = ({
  ...
  const getRowColor = (paymentLists) => {
    if (
      ...
      paymentLists.some(
        (payment) => payment.isPaySuccess == "1" || payment.isPaySuccess == "7"
      )
    ) {
      return "bg-green-100";
    }
                      ...
                      {!isCash ? (
                        <td className="border p-2 font-light text-right">
                          <button
                            ...
                            className="flex items-center justify-center w-full text-yellow-400 hover:text-yellow-500 focus:outline-none transition-colors duration-200"
                          >
                            <span className="mr-2">
