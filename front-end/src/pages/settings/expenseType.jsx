...
import TableExpenseType from "src/components/tables/tableExpenseType";
import ModalExpenseType from "src/components/modals/modalExpenseType";
import {
  addExpenseType,
  fetchExpenseType,
  updateExpenseType,
} from "src/store/expenseType";
...
const SettingExpenseTypePage = () => {
  ...
  const store = useSelector((state) => state.expenseType);
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (isNaN(e.id)) {
      dispatch(addExpenseType(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
    } else {
      dispatch(updateExpenseType(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
              ...
              {radioTypeLists.map((radioTypeList, k) => (
                <div className="flex items-center me-4 ml-2" key={k}>
                  <input
                    ...
                    checked={radioTypeList.value === SelectType}
...
export default SettingExpenseTypePage;
