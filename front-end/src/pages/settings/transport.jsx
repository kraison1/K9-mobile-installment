...
import TableTransport from "src/components/tables/tableTransport";
import ModalTransport from "src/components/modals/modalTransport";
...
const SettingTransportPage = () => {
  ...
  const store = useSelector((state) => state.transport);
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (isNaN(e.id)) {
      dispatch(addTransport(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
    } else {
      dispatch(updateTransport(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
...
export default SettingTransportPage;
