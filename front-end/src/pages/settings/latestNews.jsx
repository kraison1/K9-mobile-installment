...
import TableLatestNews from "src/components/tables/tableLatestNews";
import ModalLatestNews from "src/components/modals/modalLatestNews";
import { useDispatch, useSelector } from "react-redux";
import {
  addLatestNews,
  fetchLatestNews,
  updateLatestNews,
} from "src/store/latestNews";
...
const LatestNews = () => {
  ...
  const TableHeaders = [
    ...
    { name: "รูปภาพ", w: 10, align: "text-center" },
    { name: "ชื่อ", w: 30, align: "text-left" },
    { name: "เนื้อหา", w: 35, align: "text-left" },
    { name: "ลิงก์", w: 20, align: "text-left" },
  ];
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (isNaN(e.id)) {
      dispatch(addLatestNews(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
    } else {
      dispatch(updateLatestNews(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
