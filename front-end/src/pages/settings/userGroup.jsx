...
import TableUserGroup from "src/components/tables/tableUserGroup";
import ModalUserGroup from "src/components/modals/modalUserGroup";
import { useDispatch, useSelector } from "react-redux";
import {
  addUserGroup,
  fetchUserGroup,
  updateUserGroup,
} from "src/store/userGroup";
...
const DefaultPermissionValues = ["listMenus"];

const SettingUserGroupPage = () => {
  const { isLoadingOpen, setIsLoadingOpen } = useAuth();
  const [Modal, setModal] = React.useState(false);
  const [Permissions, setPermissions] = React.useState(DefaultPermissionValues);
  ...
  const addRow = () => {
    setModal(true);
    setPermissions(DefaultPermissionValues);
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (isNaN(e.id)) {
      dispatch(addUserGroup(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
    } else {
      dispatch(updateUserGroup(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
...
export default SettingUserGroupPage;
