...
import TableStockProductUnit from "src/components/tables/tableStockProductUnit";
import ModalStockProductUnit from "src/components/modals/modalStockProductUnit";
import { useDispatch, useSelector } from "react-redux";
import {
  addProductUnits,
  fetchProductUnits,
  updateProductUnits,
} from "src/store/productUnit";
...
const ProductUnitPage = () => {
  ...
  const store = useSelector((state) => state.productUnit);
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (isNaN(e.id)) {
      dispatch(addProductUnits(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
    } else {
      dispatch(updateProductUnits(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
