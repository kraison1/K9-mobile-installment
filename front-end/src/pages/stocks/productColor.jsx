...
import TableStockProductColor from "src/components/tables/tableStockProductColor";
import ModalStockProductColor from "src/components/modals/modalStockProductColor";
import { useDispatch, useSelector } from "react-redux";
import {
  addProductColors,
  fetchProductColors,
  updateProductColors,
} from "src/store/productColor";
...
const ProductColorPage = () => {
  ...
  const store = useSelector((state) => state.productColor);
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (isNaN(e.id)) {
      dispatch(addProductColors(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
    } else {
      dispatch(updateProductColors(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
