...
import TableStockProductStorage from "src/components/tables/tableStockProductStorage";
import ModalStockProductStorage from "src/components/modals/modalStockProductStorage";
import { useDispatch, useSelector } from "react-redux";
import {
  addProductStorages,
  fetchProductStorages,
  updateProductStorages,
} from "src/store/productStorage";
...
const ProductStoragePage = () => {
  ...
  const store = useSelector((state) => state.productStorage);
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (isNaN(e.id)) {
      dispatch(addProductStorages(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
    } else {
      dispatch(updateProductStorages(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
