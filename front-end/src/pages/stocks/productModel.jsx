...
import TableStockProductModel from "src/components/tables/tableStockProductModel";
import ModalStockProductModel from "src/components/modals/modalStockProductModel";
import { useDispatch, useSelector } from "react-redux";
import {
  addProductModels,
  fetchProductModels,
  updateProductModels,
} from "src/store/productModel";
...
const ProductModelPage = () => {
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (isNaN(e.id)) {
      dispatch(addProductModels(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
    } else {
      dispatch(updateProductModels(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
