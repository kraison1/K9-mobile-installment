...
import TableProcessSaving from "src/components/tables/tableProcessSaving";
import ModalProcessSaving from "src/components/modals/modalProcessSaving";
import {
  fetchProcessSaving,
  updateProcessSaving,
} from "src/store/processSaving";
...
const ProcessSavingPage = () => {
  ...
  const editRow = (e) => {
    if (e.savingType == 4) {
      ...
      if (res.status == "1") {
        ...
        setRowDataSale({
          ...DefaultValuesDowns,
          payType: productSaving.payType,
          bankId: productSaving.bankId,
          productSavingId: productSaving.id,
          customerId: productSaving?.customerId || "",
          product: productSaving?.product || {},
          saleType: "3",
          useCalType: "2",
        ...
        warning(
          `คุณต้องดูที่รายการสัญญา ค้นหา: ${
            productSaving.product?.imei || `${productSaving.customer?.name} ${productSaving.customer?.lastname}`
          }`
        );
      } else {
  ...
  const submitRow = (e) => {
    setIsLoadingOpen(true);
    if (!isNaN(e.id)) {
      dispatch(updateProcessSaving(e))
        .unwrap()
        .then(() => setIsLoadingOpen(false))
        .catch(() => setIsLoadingOpen(false));
      setModal(false);
...
export default ProcessSavingPage;
